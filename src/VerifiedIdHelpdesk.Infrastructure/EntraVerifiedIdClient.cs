...
public class EntraVerifiedIdClient : IVerifiedIdClient
{
    ...
    public EntraVerifiedIdClient(IConfiguration config, ILogger<EntraVerifiedIdClient> logger)
    {
        _config = config;
        _logger = logger;
        ...
        {
            BaseAddress = new Uri(config["VerifiedId:RequestServiceBaseUrl"]
    ...
    private async Task<T> PostAsync<T>(string url, object body)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new HttpRequestException($"VerifiedId API returned {response.StatusCode}: {responseBody}");
        }
